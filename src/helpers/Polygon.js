...
const appendEdgeFor = (
  ...
  polygon,
  options,
  { parts, newPoint, startPoint, endPoint }
) => {
...
export const createFor = (
  ...
  const concavedLatLngs =
    !preventMutations && options.concavePolygon
      ? concavePolygon(map, latLngs)
      : latLngs;

  // Simplify the polygon before adding it to the map.
  const addedPolygons = limitReached
    ? []
    : map.simplifyPolygon(concavedLatLngs).map(latLngs => {
  ...
  if (
    !limitReached &&
    !preventMutations &&
    polygons.get(map).size > 1 &&
    options.mergePolygons
  ) {
    // Attempt a merge of all the polygons if the options allow, and the polygon count is above one.
    const addedMergedPolygons = mergePolygons(
      map,
      Array.from(polygons.get(map)),
      options
    );
    console.log(addedMergedPolygons);
...
export const clearFor = map => {
  Array.from(polygons.get(map).values()).forEach(polygon =>
...
export default (map, polygon, options) => {
  return event => {
    ...
    const newPoint = map.mouseEventToContainerPoint(
    ...
    const appendEdge = () =>
      appendEdgeFor(map, polygon, options, {
        parts,
        newPoint,
        startPoint,
        endPoint
